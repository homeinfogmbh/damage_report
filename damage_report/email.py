...
def get_emails(damage_report: Union[DamageReport, int]) -> Iterator[EMail]:
    ...
    damage_report = (
        DamageReport.select(cascade=True).where(DamageReport.id == damage_report).get()
    )

    for notification_email in NotificationEmail.select().where(
        NotificationEmail.customer == damage_report.customer
    ):
        yield create_email(damage_report, notification_email)


def create_email(
    damage_report: DamageReport, notification_email: NotificationEmail
) -> EMail:
    """Creates an email for a damage report."""
    ...
    subject = subject.format(
        damage_type=damage_report.damage_type, address=damage_report.address
    )
    message = (
